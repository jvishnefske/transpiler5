...
private:
  ...
  /// module-level `emitrust.struct_def`. `cType` is the C type walked in
  /// parallel: a data-pointer field (stored as an i64 cursor member,
...
CImporter::emitAggregateInitList(Value place, Type type,
  ...
  if (llvm::isa<emitrust::StructType>(type)) {
    ...
    for (const clang::FieldDecl *field : record->fields()) {
      ...
      Location elementLoc = translateLoc(element->getBeginLoc());
      if (isDataPointer(field->getType())) {
        // A data-pointer field's binding was recorded by the analysis
        // walk of this declaration; the stored i64 member keeps its
        // default 0 (the degenerate binding carries no runtime
        // information), so a supported initializer emits nothing.
        if (!instance)
