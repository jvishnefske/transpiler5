...
private:
  ...
  /// a dereference or subscript through a tracked pointer. A global data
  /// pointer at the root is tracked on first sight (like the arithmetic
  /// and rebinding forms), so a body whose only mention of the global is a
  /// write through it still contributes the write to the program-wide
  /// facts — a read-only literal-backed global region must reject it.
