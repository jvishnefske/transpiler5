...
/// cross-TU shape-dedup purposes, exactly like a tagged struct. This is the
/// base spelling only: `CImporter::structSymbolName` layers the tag-versus-
/// ordinary-namespace collision renaming on top and is what every symbol
/// and type mention must go through.
...
FailureOr<Type> CImporter::mapType(clang::QualType type, Location loc) {
  ...
  if (const auto *record =
    ...
    FailureOr<std::string> structName = structSymbolName(definition, loc);
    if (failed(structName))
      return failure();
    if (structName->empty())
...
LogicalResult CImporter::importRecord(const clang::RecordDecl *record,
  ...
  if (!importedRecords.insert(definition).second)
    return success();
  FailureOr<std::string> assignedName = structSymbolName(definition, defLoc);
  if (failed(assignedName))
    return failure();
  llvm::StringRef structName = *assignedName;
  if (structName.empty())
