  ...
  static const struct {
    ...
    llvm::StringRef source;
      ...
      {"__emitrust_scan_u",
       ...
       "    s + 1\n"
       "}"},
      {"__emitrust_scan_float_word",
       "/// The `inf` / `infinity` / `nan` arm of C's float grammar, entered\n"
       "/// on the first character and matched case-insensitively.\n"
       "///\n"
       "/// The MEASURED glibc edges this reproduces, none of them guessable:\n"
       "///   `INFX`      -> matches `inf`, and `X` is pushed BACK;\n"
       "///   `INFIXY`    -> a MATCHING FAILURE, and the offending `X` is\n"
       "///                  CONSUMED, leaving only `Y` -- past the `inf`\n"
       "///                  boundary the keyword scanner does not push back,\n"
       "///                  and the residual stream is observable to whatever\n"
       "///                  reads next;\n"
       "///   `INFINIT`   -> a matching failure at end of file, NOT `inf`:\n"
       "///                  the scan is greedy and never backtracks;\n"
       "///   `nanQ`      -> matches `nan`, and `Q` is pushed back.\n"
       "fn __emitrust_scan_float_word(text: &mut String, first: i32) -> i32 {\n"
       "    let nan = first == 110 || first == 78;\n"
       "    let word: &[u8] = if nan { b\"nan\" } else { b\"infinity\" };\n"
       "    let mut matched = 1usize;\n"
       "    while matched < word.len() {\n"
       "        let c = __emitrust_stdin_getc();\n"
       "        if c >= 0 && ((c as u8) | 32) == word[matched] {\n"
       "            matched += 1;\n"
       "            continue;\n"
       "        }\n"
       "        if matched == 3 {\n"
       ...
       "        if c == 40 {\n"
       "            panic!(\n"
       "                \"scanf: a NaN payload, nan(...), is not supported\"\n"
       "            );\n"
       "        }\n"
       "        __emitrust_stdin_ungetc(c);\n"
       "        text.push_str(\"nan\");\n"
       "        return 1;\n"
       "    }\n"
       "    text.push_str(if matched == 3 { \"inf\" } else { \"infinity\" });\n"
       "    1\n"
       "}"},
      {"__emitrust_scan_float_text",
       "/// The greedy scan of C's float grammar, shared by `%f` and `%lf`\n"
       "/// because they differ only in the out-argument's type. Returns 1 with\n"
       "/// `text` holding a complete matching sequence, 0 for a matching\n"
       "/// failure and -1 for an input failure (end of file before anything).\n"
       "///\n"
       "/// The model is FR-183's: a greedy byte scanner with EXACTLY ONE\n"
       "/// character of pushback. C reads the LONGEST prefix of a matching\n"
       "/// sequence and never backtracks, so `1.2e` and `1.2e+x` are MATCHING\n"
       "/// FAILURES (the `e`, and the sign, are already consumed) rather than\n"
       "/// the value 1.2 -- both measured against glibc.\n"
       "///\n"
       "/// The collected text is deliberately a subset both C and Rust read\n"
       "/// identically: Rust's `f32`/`f64` parser and glibc's `strtof`/`strtod`\n"
       "/// are both correctly rounded per IEEE-754, so on the decimal,\n"
       "/// `inf`/`infinity` and bare `nan` forms they agree BIT FOR BIT. The\n"
       "/// two forms they do NOT agree on come from runtime input and so\n"
       "/// cannot be refused at import time; they PANIC here rather than\n"
       "/// silently producing a different number:\n"
       "///   `0x1p3` -- C's hexadecimal form, which Rust does not accept and\n"
       "///              which cannot be converted here without reimplementing\n"
       "///              libc's rounding;\n"
       "///   `nan(1)` -- a NaN payload, whose mapping into the significand is\n"
       "///              libc-specific.\n"
       "/// A loud stop is this project's documented safe direction; silently\n"
       "/// answering something else is not.\n"
       "fn __emitrust_scan_float_text(text: &mut String) -> i32 {\n"
       "    let mut c = __emitrust_scan_skip_ws();\n"
       "    if c < 0 {\n"
       "        return -1;\n"
       "    }\n"
       ...
       "        return __emitrust_scan_float_word(text, c);\n"
       "    }\n"
       "    if c == 48 {\n"
       "        let next = __emitrust_stdin_getc();\n"
       "        if next == 120 || next == 88 {\n"
       "            panic!(\n"
       "                \"scanf: hexadecimal floating-point input is not supported\"\n"
       "            );\n"
       "        }\n"
       "        __emitrust_stdin_ungetc(next);\n"
       "    }\n"
       ...
       "    }\n"
       "    if c == 101 || c == 69 {\n"
       "        let mut e = __emitrust_stdin_getc();\n"
       "        let mut sign = 0;\n"
       "        if e == 43 || e == 45 {\n"
       "            sign = e;\n"
       "            e = __emitrust_stdin_getc();\n"
       "        }\n"
       "        if !(48..=57).contains(&e) {\n"
       "            __emitrust_stdin_ungetc(e);\n"
       "            return 0;\n"
       "        }\n"
       "        text.push('e');\n"
       "        if sign != 0 {\n"
       ...
       "        }\n"
       "        while (48..=57).contains(&e) {\n"
       "            text.push(e as u8 as char);\n"
       "            e = __emitrust_stdin_getc();\n"
       "        }\n"
       "        __emitrust_stdin_ungetc(e);\n"
       "    } else {\n"
       ...
       "}"},
      {"__emitrust_scan_f",
       "/// `%f` (and its C-identical spellings `%a`, `%e`, `%g` in either\n"
       "/// case): the greedy float scan, then Rust's correctly-rounded `f32`\n"
       "/// parser -- which agrees with glibc's `strtof` bit for bit on every\n"
       "/// form the scanner admits. The value is written ONLY on a complete\n"
       "/// match; a matching failure leaves the C object untouched, exactly\n"
       "/// as `%d` does.\n"
       "fn __emitrust_scan_f(s: i32, out: &mut f32) -> i32 {\n"
       "    if s < 0 {\n"
       "        return s;\n"
       "    }\n"
       "    let mut text = String::new();\n"
       "    match __emitrust_scan_float_text(&mut text) {\n"
       "        1 => {\n"
       "            *out = match text.parse::<f32>() {\n"
       "                Ok(v) => v,\n"
       "                Err(_) => panic!(\"scanf: cannot convert {}\", text),\n"
       "            };\n"
       "            s + 1\n"
       "        }\n"
       "        0 => -s - 2,\n"
       ...
       "}"},
      {"__emitrust_scan_lf",
       "/// `%lf`: the SAME scan and the same correctly-rounded parser, over a\n"
       "/// `double` out-reference. The `l` is the only thing C's float family\n"
       "/// spellings actually select.\n"
       "fn __emitrust_scan_lf(s: i32, out: &mut f64) -> i32 {\n"
       "    if s < 0 {\n"
       "        return s;\n"
       "    }\n"
       "    let mut text = String::new();\n"
