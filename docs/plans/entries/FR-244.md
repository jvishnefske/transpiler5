- [x] FR-244 (opened 2026-09-11, blocked by FR-243): **CppStdSuite:
  ...
  *.reference_output (missing for some programs, may carry harness-appended
  ...
  LANDED 2026-09-11 over the two planned commits, submodule pinned at
  4b2c892865cb (shallow). THE FILED CORPUS ESTIMATE WAS WRONG BY 3x AND
  THE MEASUREMENT IS THE RECORD: the three chosen trees contain only 89
  .cpp files total (Regression/C++ 31, UnitTests 51, Misc-C++ 7), so
  the 100-300 survivor target was unreachable from the start. Curation
  measured: 65 survivors -- 17 dropped in four harness-knob directories
  (Integer, SignlessTypes, SignlessTypes/Large, Misc-C++/Large -- all
  RUN_OPTIONS/HASH_PROGRAM_OUTPUT users), 5 fail a bare -std=c++17 build
  (three dynamic-exception-spec C++17 removals, two AMX target-feature
  tests), 2 exceed the 2s native-runtime ceiling (both stepanov
  benchmarks). curate_candidates.py --emit-expected also writes the 65
  .expected references so a pin advance is one command.
  DAY ONE, MEASURED VIA --update: **65 candidates, 11 PASS, 0
  MISCOMPILE, 54 UNSUPPORTED** -- no quarantine entries and no defect FR
  needed. The 11: mandel-text, array-init, Crasher, EnumSizeProblem,
  Bitfield, global_type, matrix-types-spec, and the four block-* tests.
  A from-11 ratchet is the frontier instrument the saturated CTestSuite
  (220/220) and Cpp17Suite (35/35) no longer provide.
  FAST-TIER WALL TIME: 32.98s with the ledger live (1094 discovered
  tests) against a 29-31s baseline -- roughly +2-4s, nowhere near
  dominating, so the third-tier tripwire stays untripped. The ledger
  runs in the heavy-harness parallelism group from its first commit.
  CLAUDE.md's worktree rule now inits BOTH submodules with a
  --reference note (~2.1G checkout); spike-worktree-setup.sh inits no
  submodules (checked), so it needed no change. Full gate green.
