- [x] FR-243 (opened 2026-09-11): **ONE LEDGER RUNNER FOR THREE SUITES:
  ...
  LANDED 2026-09-11. BIT-IDENTITY MEASURED, NOT ASSUMED: old runners
  (from git HEAD) and new wrappers run side by side against the same
  build/tools/emitrust-cc -- non-update stdout byte-identical for both
  suites; --update into scratch manifest copies byte-identical for both
  (headers included, Cpp17's tasks.toml sentence verbatim); both scratch
  manifests also byte-identical to the repo manifests, which were never
  touched. CTestSuite 220/220 and Cpp17Suite 35/35 through the shared
  body. ONE CONSUMER THE ENTRY MISSED, FOUND BY GREP BEFORE IT COULD
  BITE: test/Fuzz/differ.py and fuzz_differential.py import six helpers
  FROM run_c_testsuite -- the wrapper re-exports exactly those names and
  the import was executed, not assumed. SIDE-CHANNEL VERIFIED END TO END
  through lit's curated environment: a full gate run with
  EMITRUST_LEDGER_SUMMARY_DIR set produced c-testsuite.txt and cpp17.txt
  whose content equals the runners' stdout report. Candidates/expected-dir
  modes land dormant (no suite uses them until FR-244); the candidates
  path adds a sanitized-workdir-collision guard because relative paths,
  unlike NNNNN stems, can collide after sanitization. Full gate green.
