...
// the spurious `mut` cleared, FR-61b's `computeIfExprBindings` becomes eligible
...
// correct; @index_no_lift is the same shape with one statement between the
// declaration and the `if`, which is out of the lift's reach (the lift requires
// `op->getNextNode()`), so it pins the `mut` decision on its own.
//
...
// A statement sits between the declaration and the `if`, so the if-expression
// lift does not apply and the two-program-point rendering survives -- which is
// precisely what makes this the isolated `mut` pin.
// CHECK-LABEL: fn index_no_lift(v0: bool, v1: &mut [i32], v2: i64) -> i64 {
...
// CHECK-NEXT:    v4
// CHECK-NEXT:  }
emitrust.func @index_no_lift(%c: i1, %arr: !emitrust.mut_ref<!emitrust.slice<i32>>, %x: i64) -> i64 {
